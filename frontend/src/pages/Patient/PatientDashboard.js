    ...
    const fetchData = async () => {
      try {
        ...
        if (errorMsg.includes('Token expired') || errorMsg.includes('Token verification error')) {
          ...
          navigate('/login', { state: { message: t('session_expired') }, replace: true });
        }
      } finally {
        setLoading(false);
      }
      ...
      <div className="flex flex-col min-h-screen bg-gradient-to-b from-blue-50 to-gray-100">
        ...
        <div className="flex items-center justify-center flex-grow">
          <div className="flex items-center space-x-3">
            ...
            <p className="text-lg text-blue-700 animate-pulse">{t('loading_dashboard')}</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex-grow p-6 mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-6 mb-12 md:grid-cols-3">
          <div className="p-6 transition-all duration-300 bg-white shadow-lg rounded-xl hover:shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-blue-700">{t('food_logs')}</h2>
          ...
          <div className="p-6 transition-all duration-300 bg-white shadow-lg rounded-xl hover:shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-blue-700">{t('messages')}</h2>
          ...
          <div className="p-6 transition-all duration-300 bg-white shadow-lg rounded-xl hover:shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-blue-700">{t('goals')}</h2>
              <Target className="w-6 h-6 text-blue-500" />
            </div>
            <p className="text-3xl font-bold text-gray-700">{t('coming_soon')}</p>
            <button
        ...
        <div className="grid grid-cols-1 gap-6 mb-12 lg:grid-cols-2">
          <div className="p-6 bg-white shadow-lg rounded-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-blue-700">{t('recent_food_logs')}</h2>
              <Utensils className="w-6 h-6 text-blue-500" />
            </div>
            {foodLogs.length === 0 ? (
          ...
          <div className="p-6 bg-white shadow-lg rounded-xl">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-blue-700">{t('recent_messages')}</h2>
              <MessageSquare className="w-6 h-6 text-blue-500" />
            </div>
            {messages.length === 0 ? (
