// frontend/src/pages/ProviderMessagesPage.js
import { useEffect, useState, useCallback, useRef } from "react";
import { useTranslation } from "react-i18next";
import { MessageSquare, AlertCircle, Send, Inbox } from "lucide-react";
...
const ProviderMessagesPage = () => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      ...
      // Mark unread messages as read for the selected conversation
      if (selectedConversation) {
        ...
        if (unreadMessages.length > 0) {
          ...
          await fetchMessages(); // Refresh after marking as read
        }
      }
    } catch (err) {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const res = await api.post(`/provider/message/${patientId}`, { content: newMessage.trim() });
