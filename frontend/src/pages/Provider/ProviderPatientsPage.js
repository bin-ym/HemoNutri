  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const errorMsg = err.response?.data?.error || 'Failed to load patients';
        setError(errorMsg);
      ...
      <div className="flex flex-col min-h-screen bg-gradient-to-b from-teal-50 to-gray-100">
        ...
        <div className="flex items-center justify-center flex-grow">
          <div className="flex items-center space-x-3">
            ...
            <p className="text-lg text-teal-700 animate-pulse">Loading patients...</p>
          </div>
        </div>
      </div>
    );
        ...
        <section className="mb-12">
          <div className="p-6 bg-white shadow-lg rounded-xl">
            <div className="flex items-center justify-between pb-4 mb-6 border-b-2 border-teal-100">
              <h2 className="text-2xl font-semibold text-teal-600">Assigned Patients</h2>
            ...
            {patients.length === 0 ? (
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {patients.map((patient) => (
                  <div
                    key={patient._id}
                    className="p-4 transition-all duration-300 border border-teal-100 rounded-lg shadow-md cursor-pointer bg-teal-50 hover:shadow-xl"
                  ...
                  >
                    ...
                    <button
                      ...
                      className="px-3 py-1 mt-2 text-white transition duration-300 bg-teal-600 rounded hover:bg-teal-700"
                    >
                      View Details
                    </button>
                  </div>
